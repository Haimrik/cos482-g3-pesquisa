...
@Component({
    selector: 'jhi-usuario-cos-482-g-3',
    templateUrl: './usuario-cos-482-g-3.component.html'
})
export class UsuarioCos482G3Component implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/usuario-cos-482-g-3'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/usuario-cos-482-g-3', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: UsuarioCos482G3) {
        return item.id;
    }
