...
@Component({
    selector: 'jhi-aluno-cos-482-g-3',
    templateUrl: './aluno-cos-482-g-3.component.html'
})
export class AlunoCos482G3Component implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/aluno-cos-482-g-3'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/aluno-cos-482-g-3', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: AlunoCos482G3) {
        return item.id;
    }
